...
async function initializeApp() {
    const initId = 'init-app';
    
    try {
        // Mostrar preloader de inicialização
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(initId, {
        ...
        // Ocultar preloader após inicialização
        if (window.GlobalPreloader) {
            window.GlobalPreloader.hide(initId);
        }

    } catch (error) {
        console.error('Erro ao carregar dados do usuário:', error);
        
        // Ocultar preloader em caso de erro
        if (window.GlobalPreloader) {
            window.GlobalPreloader.hide(initId);
        }
...
function setupEventListeners() {
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.sound-modal') && !e.target.closest('#soundBtn')) {
            closeSoundPanel();
        }
    });
}
...
function updateConnectionStatus() {
    ...
    const isWebSocketConnected = window.wsClient && window.wsClient.isConnected;
...
async function syncData() {
    ...
    const syncLoaderId = 'data-sync';
    const connectionBtn = document.getElementById('connectionStatus');
    const statusText = connectionBtn?.querySelector('.btn-text');
    const statusDot = connectionBtn?.querySelector('.status-dot');
    
    try {
        // Mostrar preloader para sincronização
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(syncLoaderId, {
        ...
        // Atualizar progresso
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(syncLoaderId, 30, 'Conectando ao servidor...');
        }
        ...
        // Atualizar progresso
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(syncLoaderId, 70, 'Sincronizando agendamentos...');
        }
        ...
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(syncLoaderId, 100, 'Sincronização concluída!');
            setTimeout(() => {
                window.GlobalPreloader.hide(syncLoaderId);
            }, 500);
        }
        ...
        showToast('Falha na sincronização. Contate o suporte se persistir.', 'error');
        updateConnectionStatus();
...
function setupWindowControls() {
    ...
    // Configurar evento F11 para tela cheia (desabilitado no renderer; tratado no processo principal para maximizar)
    // document.addEventListener('keydown', (e) => {
    //     if (e.key === 'F11') {
    //         e.preventDefault();
    //         console.log('F11 pressionado - alternando tela cheia');
    //         console.log('Classe fullscreen-mode antes:', document.body.classList.contains('fullscreen-mode'));
    ...
    if (ipcRenderer) {
        ipcRenderer.on('fullscreen-changed', (event, isFullScreen) => {
            console.log('Estado de tela cheia alterado:', isFullScreen);
            console.log('Aplicando classe fullscreen-mode:', isFullScreen);
            document.body.classList.toggle('fullscreen-mode', isFullScreen);
            console.log('Classe fullscreen-mode após aplicar:', document.body.classList.contains('fullscreen-mode'));
            console.log('Classes do body:', document.body.className);
            
            // Verificar se a barra de janela está sendo ocultada
            const windowBar = document.querySelector('.custom-window-bar');
            if (windowBar) {
                const computedStyle = window.getComputedStyle(windowBar);
                console.log('Display da barra de janela:', computedStyle.display);
                console.log('Visibility da barra de janela:', computedStyle.visibility);
                console.log('Opacity da barra de janela:', computedStyle.opacity);
            }
        });
    }
}
...
async function handleCreateAgendamento(e) {
    ...
    const motorista = formData.get('motorista') || document.getElementById('motorista').value;
    ...
    // Salvar motorista no sistema se foi informado
    if (motorista && cidade && window.motoristaAutocomplete) {
        window.motoristaAutocomplete.salvarMotorista(cidade, motorista);
    }
    
    const createId = 'create-agendamento';
    
    try {
        // Mostrar preloader
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(createId, {
        ...
        if (result.success) {
            ...
            if (window.voiceManager && !window.soundMuted && window.soundManager && window.soundManager.isEnabled()) {
            ...
            if (window.webSocketClient && window.webSocketClient.isConnected) {
                window.webSocketClient.socket.emit('agendamento:create', {
        ...
        // Ocultar preloader
        if (window.GlobalPreloader) {
            window.GlobalPreloader.hide(createId);
        }
    } catch (error) {
        console.error('Erro ao criar agendamento:', error);
        showToast('Falha ao criar agendamento. Entre em contato com o administrador.', 'error');
...
async function loadAgendamentos() {
    const loaderId = 'load-agendamentos';
    
    try {
        // Mostrar preloader global
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(loaderId, {
        ...
        // Ocultar preloader
        if (window.GlobalPreloader) {
            window.GlobalPreloader.hide(loaderId);
        }
    } catch (error) {
...
function filterAgendamentos() {
    ...
    switch (currentTab) {
        case 'todos':
            // Mostrar todos os agendamentos que não estão concluídos nem cancelados
            filtered = sourceAgendamentos.filter(a => a.status !== 'Concluído' && a.status !== 'Cancelado');
            console.log('Todos os agendamentos (exceto concluídos e cancelados):', filtered.length);
            break;
        case 'concluidos':
...
function createAgendamentoCard(agendamento) {
    ...
    const observacoes = agendamento.observacoes ? 
        `<div class="postit-row postit-observacao">
            ...
            <span><b>Observações:</b> <span class="observacao-text" title="${agendamento.observacoes}">${agendamento.observacoes}</span></span>
        </div>` : '';
        ...
        <div class="agendamento-card ${statusClass} ${postitStyle} ${shareClass}" data-id="${agendamento.id}" data-shared="${isShared}">
            ...
            <div class="postit-content">
                ...
                <div class="postit-row">
                    ...
                    <span class="postit-label">Cidade:</span> <span class="postit-value cidade" title="${agendamento.cidade}">${agendamento.cidade}</span>
                </div>
                <div class="postit-row">
                ...
                ${agendamento.numeroContato ? `
                    <div class="postit-row">
                        <span class="icon"><i class="fa-solid fa-phone"></i></span>
                        <span class="postit-label">Telefone:</span> <span class="postit-value">${agendamento.numeroContato}</span>
                    </div>
                ` : ''}
                ${agendamento.motorista ? `
                    <div class="postit-row">
                        <span class="icon"><i class="fa-solid fa-id-card"></i></span>
                        <span class="postit-label">Motorista:</span> <span class="postit-value">${agendamento.motorista}</span>
                    </div>
                ` : ''}
            ...
            <div class="postit-footer">
                ${agendamento.status === 'Concluído' ? `
                    ...
                    ${agendamento.lastEditedBy ? `
                        <div class="postit-editor-info">
                            <span class="editor-label">Última edição por:</span>
                            <span class="editor-name">${agendamento.lastEditedBy}</span>
                            <span class="edit-date">${new Date(agendamento.lastEditedAt).toLocaleString('pt-BR')}</span>
                        </div>
                    ` : ''}
                    ${agendamento.editHistory && agendamento.editHistory.length > 0 ? `
                        <div class="postit-edit-history">
                            <button class="edit-history-toggle" onclick="toggleEditHistory('${agendamento.id}')">
                                <i class="fa-solid fa-history"></i> Ver Histórico de Edições (${agendamento.editHistory.length})
                            </button>
                            <div class="edit-history-content" id="edit-history-${agendamento.id}" style="display: none;">
                                ${agendamento.editHistory.map(edit => `
                                    <div class="edit-entry">
                                        <div class="edit-header">
                                            <span class="edit-user">${edit.editedBy}</span>
                                            <span class="edit-date">${new Date(edit.editedAt).toLocaleString('pt-BR')}</span>
                                        </div>
...
// Alternar exibição do histórico de edições
function toggleEditHistory(agendamentoId) {
    const historyContent = document.getElementById(`edit-history-${agendamentoId}`);
    const toggleButton = historyContent.previousElementSibling;
    
    if (historyContent.style.display === 'none') {
        historyContent.style.display = 'block';
        toggleButton.innerHTML = '<i class="fa-solid fa-history"></i> Ocultar Histórico';
    } else {
        historyContent.style.display = 'none';
        const editCount = historyContent.querySelectorAll('.edit-entry').length;
        toggleButton.innerHTML = `<i class="fa-solid fa-history"></i> Ver Histórico de Edições (${editCount})`;
    }
}
...
async function concluirAgendamento(id) {
    try {
        ...
        showToast('Falha ao concluir agendamento. Contate o suporte técnico.', 'error');
    }
}
...
async function deleteAgendamento(id) {
    try {
        ...
        if (result.isConfirmed) {
            const deleteLoaderId = 'delete-agendamento';
            
            try {
                // Mostrar preloader para exclusão
                if (window.GlobalPreloader) {
                    window.GlobalPreloader.show(deleteLoaderId, {
                        message: 'Excluindo agendamento...',
                        type: 'spinner'
                    });
                }
                ...
                // Enviar notificação WebSocket para outros usuários
                if (window.webSocketClient && window.webSocketClient.isConnected) {
                    window.webSocketClient.sendAgendamentoUpdate('delete', { id });
                    console.log('[WEBSOCKET] Notificação de agendamento excluído enviada');
                }
                
                // Ocultar preloader
                if (window.GlobalPreloader) {
                    window.GlobalPreloader.hide(deleteLoaderId);
                }
...
function openEditModal(id) {
    ...
    document.getElementById('editMotorista').value = agendamento.motorista || '';
...
async function handleEditAgendamento(e) {
    ...
    if (!editingAgendamento) {
        ...
        showToast('Nenhum agendamento selecionado. Selecione um item para editar.', 'error');
        return;
    }
    
    console.log('Editing agendamento:', editingAgendamento);
    
    const editLoaderId = 'edit-agendamento';
    
    try {
        // Mostrar preloader para edição
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(editLoaderId, {
        ...
        // Obter usuário atual
        const currentUser = window.currentUser || { displayName: 'Usuário Desconhecido', username: 'unknown' };
        
        // Criar histórico de edição
        const editHistory = editingAgendamento.editHistory || [];
        const editRecord = {
        ...
        // Detectar mudanças e registrar no histórico
        const fieldsToCheck = {
        ...
        for (const [field, newValue] of Object.entries(fieldsToCheck)) {
            const oldValue = editingAgendamento[field] || '';
        ...
        // Verificar mudanças nas paradas
        const oldParadas = editingAgendamento.paradas || [];
        if (JSON.stringify(oldParadas) !== JSON.stringify(paradas)) {
            editRecord.changes.push({
                field: 'paradas',
                oldValue: oldParadas.join(', ') || 'Nenhuma',
                newValue: paradas.join(', ') || 'Nenhuma'
            });
        }
        
        // Adicionar registro ao histórico apenas se houve mudanças
        ...
        const updatedData = {
            ...
            lastEditedBy: editRecord.changes.length > 0 ? currentUser.displayName : editingAgendamento.lastEditedBy,
            lastEditedAt: editRecord.changes.length > 0 ? new Date().toISOString() : editingAgendamento.lastEditedAt
        };
        
        // Salvar motorista se fornecido
        if (updatedData.motorista && updatedData.cidade) {
            window.motoristaAutocomplete.salvarMotorista(updatedData.cidade, updatedData.motorista);
        }
        ...
        // Enviar notificação WebSocket para outros usuários
        if (window.webSocketClient && window.webSocketClient.isConnected) {
            window.webSocketClient.sendAgendamentoUpdate('update', updatedData);
            console.log('[WEBSOCKET] Notificação de agendamento atualizado enviada');
        }
        
        // Ocultar preloader
        if (window.GlobalPreloader) {
            window.GlobalPreloader.hide(editLoaderId);
        }
        ...
        if (window.voiceManager && !window.soundMuted && window.soundManager && window.soundManager.isEnabled()) {
            window.voiceManager.speakAgendamentoAtualizado(
                updatedData.nomeCliente,
                updatedData.horario,
                updatedData.cidade
            );
        }
...
async function handleCancelAgendamento(e) {
    ...
    const cancelLoaderId = 'cancel-agendamento';
    
    try {
        // Mostrar preloader para cancelamento
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(cancelLoaderId, {
        ...
        await ipcRenderer.invoke('updateAgendamento', cancelData);
        
        // Enviar notificação WebSocket para outros usuários
        if (window.webSocketClient && window.webSocketClient.isConnected) {
            window.webSocketClient.sendAgendamentoUpdate('cancel', {
                ...agendamentoParaCancelar,
                ...cancelData
            });
            console.log('[WEBSOCKET] Notificação de agendamento cancelado enviada');
        }
        
        // Ocultar preloader
        if (window.GlobalPreloader) {
            window.GlobalPreloader.hide(cancelLoaderId);
        }
...
function toggleSoundPanel() {
    const panel = document.getElementById('soundModal');
...
function toggleSoundMute() {
    ...
    if (window.soundMuted) {
        ...
        soundBtn.innerHTML = `
            ...
            <span class="btn-text">Som Desativado</span>
        `;
...
function closeSoundPanel() {
    const soundPanel = document.getElementById('soundModal');
    if (soundPanel) {
...
function initializeSoundButtonState() {
    ...
    // Verificar se o som está desabilitado (soundMuted ou soundManager desabilitado)
    const isSoundDisabled = window.soundMuted || (window.soundManager && !window.soundManager.isEnabled());
        ...
        if (agendamento.data === today && 
            ...
            if (minutesLate > 0) {
                ...
                if (shouldNotify) {
                    ...
                    if (window.voiceManager && window.soundManager && window.soundManager.isEnabled()) {
...
function openLocationModal(agendamentoId) {
    ...
    if (!modal || !citySpan) {
        showToast('Falha ao carregar localização. Contate o administrador.', 'error');
        return;
    }
...
async function initializeMapWithAddresses(enderecoOrigem, enderecoDestino, paradas = []) {
    ...
    const mapLoaderId = 'map-loader';
    
    try {
        // Mostrar preloader para carregamento do mapa
        if (window.GlobalPreloader) {
            window.GlobalPreloader.show(mapLoaderId, {
        ...
        if (typeof L === 'undefined') {
            ...
            showToast('Serviço de mapa indisponível. Entre em contato com o suporte.', 'error');
            if (window.GlobalPreloader) {
                window.GlobalPreloader.hide(mapLoaderId);
            }
            return;
        }
        ...
        // Atualizar progresso
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(mapLoaderId, 20, 'Inicializando mapa...');
        }
        ...
        if (!mapElement) {
            ...
            showToast('Falha no carregamento do mapa. Contate o administrador.', 'error');
            if (window.GlobalPreloader) {
                window.GlobalPreloader.hide(mapLoaderId);
            }
            return;
        ...
        // Atualizar progresso
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(mapLoaderId, 40, 'Geocodificando endereços...');
        }
        ...
        // Atualizar progresso
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(mapLoaderId, 60, 'Criando mapa...');
        }
        ...
        // Atualizar progresso
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(mapLoaderId, 80, 'Adicionando marcadores...');
        }
        ...
        // Finalizar preloader
        if (window.GlobalPreloader) {
            window.GlobalPreloader.updateProgress(mapLoaderId, 100, 'Mapa carregado!');
            setTimeout(() => {
                window.GlobalPreloader.hide(mapLoaderId);
            }, 500);
        }
...
async function initializeWebSocket() {
    try {
        ...
        if (connected && window.currentUser) {
            // Autenticar usuário (com JWT se disponível)
            await window.wsClient.authenticate(
...
function setupWebSocketEventHandlers() {
    ...
    // Eventos de conexão
    window.wsClient.socket.on('connect', () => {
        console.log('[WebSocket] Conectado');
        updateConnectionStatus();
        
        // Configurar sincronização de motoristas quando conectar
        setupMotoristasSync();
    });

    window.wsClient.socket.on('disconnect', () => {
...
function handleWebSocketAgendamentoUpdate(data) {
    ...
    // Não processar se for o próprio usuário que fez a alteração
    if (data.updatedBy && data.updatedBy.userId === window.currentUser?.id) {
        return;
    }
    ...
    switch (action) {
        case 'created':
            ...
            if (!agendamentos.find(a => a.id === agendamento.id)) {
                ...
                filterAgendamentos();
                showToast(`Novo agendamento: ${agendamento.nomeCliente || agendamento.cliente}`, 'info');
            }
            break;
        ...
        case 'update':
            ...
            if (index !== -1) {
                ...
                filterAgendamentos();
                showToast(`Agendamento atualizado: ${agendamento.nomeCliente || agendamento.cliente}`, 'info');
            }
            break;
            
        case 'cancel':
            // Atualizar status para cancelado
            const cancelIndex = agendamentos.findIndex(a => a.id === agendamento.id);
            if (cancelIndex !== -1) {
                agendamentos[cancelIndex] = { ...agendamentos[cancelIndex], ...agendamento };
                filterAgendamentos();
                showToast(`Agendamento cancelado: ${agendamento.nomeCliente || agendamento.cliente}`, 'warning');
            }
            break;
        ...
        case 'deleted':
            ...
            if (deleteIndex !== -1) {
                const removedAgendamento = agendamentos[deleteIndex];
                agendamentos.splice(deleteIndex, 1);
                filterAgendamentos();
                showToast(`Agendamento removido: ${removedAgendamento.nomeCliente || removedAgendamento.cliente}`, 'info');
                
                // Remover da interface se existir
                const cardElement = document.querySelector(`[data-id="${agendamento.id}"]`);
                if (cardElement) {
                    cardElement.style.transform = 'scale(0.8)';
                    cardElement.style.opacity = '0';
                    setTimeout(() => {
...
/**
 * Configurar sincronização de motoristas com WebSocket
 */
function setupMotoristasSync() {
    if (!window.motoristaManager || !window.wsClient) {
        console.warn('[MOTORISTAS] MotoristaManager ou WebSocketClient não disponível');
        return;
    }

    try {
        // Configurar sincronização WebSocket no motoristaManager
        window.motoristaManager.setupWebSocketSync(window.wsClient);
        
        // Solicitar sincronização inicial
        setTimeout(() => {
            window.motoristaManager.requestInitialSync();
        }, 1000); // Aguardar 1 segundo para garantir que a conexão está estável
        
        console.log('[MOTORISTAS] Sincronização WebSocket configurada');
        
    } catch (error) {
        console.error('[MOTORISTAS] Erro ao configurar sincronização:', error);
    }
}

/**
 * Forçar sincronização de motoristas
 */
function forceMotoristasSync() {
    if (window.motoristaManager) {
        window.motoristaManager.forceSync();
    }
}

// Expor função globalmente para uso em outros scripts
window.setupMotoristasSync = setupMotoristasSync;
window.forceMotoristasSync = forceMotoristasSync;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // REMOVIDO: carregamento dinâmico já que o script já está no HTML
    // setTimeout(() => {
...
// Função para inicializar sistema de otimização - REMOVIDA
...
//         script.src = '../utils/performance-cache.js';
//         script.async = true;
//         
//         script.onload = () => {
