...
class VoiceManager {
    constructor() {
        ...
        this.rate = parseFloat(localStorage.getItem('voiceSpeed')) || 0.9;
    ...
    selectBestVoice() {
        ...
        // Usar a voz padrão da máquina (primeira voz disponível)
        // Priorizar vozes em português brasileiro se disponível
        const ptBRVoices = voices.filter(voice => 
            voice.lang.includes('pt-BR') || voice.lang.includes('pt_BR')
        );
        
        // Fallback para português geral
        const ptVoices = voices.filter(voice => 
            voice.lang.includes('pt') && !voice.lang.includes('pt-BR')
        );
        
        // Usar voz padrão da máquina
        if (ptBRVoices.length > 0) {
            this.voice = ptBRVoices[0]; // Primeira voz pt-BR disponível
        } else if (ptVoices.length > 0) {
            this.voice = ptVoices[0]; // Primeira voz pt disponível
        } else if (voices.length > 0) {
            this.voice = voices[0]; // Primeira voz disponível (voz padrão da máquina)
        } else {
            this.voice = null;
            this.enabled = false;
        }
        
        console.log('[VoiceManager] Voz padrão da máquina selecionada:', this.voice?.name, this.voice?.lang);
    }
    ...
    addToQueue(text, options = {}) {
        ...
        if (window.soundManager && !window.soundManager.isEnabled() && !options.forceSpeak) {
            console.log('[VoiceManager] Voz não adicionada à fila porque o som está desligado');
            return;
        }
    ...
    speak(text, options = {}) {
        // Verificar se o som está habilitado no SoundManager
        if (window.soundManager && !window.soundManager.isEnabled() && !options.forceSpeak) {
            console.log('[VoiceManager] Voz desabilitada porque o som está desligado');
            return;
        }
        
        // Fallback para verificação legacy do soundMuted
    ...
    setVolume(volume) {
        ...
        console.log('[VoiceManager] Volume definido para:', this.volume);
    }
    
    setSpeed(speed) {
        this.rate = Math.max(0.1, Math.min(2, speed));
        localStorage.setItem('voiceSpeed', this.rate);
        console.log('[VoiceManager] Velocidade definida para:', this.rate);
    }
    ...
    testVoice() {
        const testText = 'Olá, esta é a voz padrão da máquina.';
        this.speak(testText, { priority: 'high' });
    }
     ...
     */
    speakAgendamentoCriado(nomeCliente, horario, cidade, atendente = null) {
        ...
        // Adicionar nome do atendente se a configuração estiver ativada
        if (atendente && window.userPreferencesManager && window.userPreferencesManager.preferences.speakAttendantName) {
            text += ` por ${atendente}`;
        }
